...
public final class DefaultToolRegistry: ToolRegistry {
    ...
    public func schemas() -> [ToolDefinition] {
        [
            ...
            ToolDefinition(name: .playMedia, description: "When Spotify is running, control Spotify directly to start playback of the currently selected playlist or track; otherwise use the standard macOS Play/Pause key for the active media player. Return a receipt promptly and do not inspect the screen afterward unless asked.", parameters: []),
            ToolDefinition(name: .playSpotifyPlaylist, description: "Play one exact Spotify playlist using a spotify:playlist URI or an open.spotify.com/playlist link. Return a receipt promptly and do not inspect the screen afterward. If the user only gives an ambiguous playlist name, ask for its Spotify share link rather than guessing.", parameters: [ToolParameter(name: "reference", type: .string, description: "Spotify playlist URI or share URL")]),
            ...
            ToolDefinition(name: .closeAllTabs, description: "After the user explicitly asks to close tabs, close every tab in the named browser. If no browser is named, close tabs in all currently running Safari, Google Chrome, Microsoft Edge, and Brave Browser windows. Requires macOS Automation permission for each browser; never close apps or submit forms.", parameters: [ToolParameter(name: "browser", type: .string, description: "Optional browser name: Safari, Google Chrome, Microsoft Edge, or Brave Browser. Omit to close tabs in all supported running browsers.", required: false)]),
