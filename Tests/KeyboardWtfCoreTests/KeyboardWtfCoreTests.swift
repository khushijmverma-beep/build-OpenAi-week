...
final class KeyboardWtfCoreTests: XCTestCase {
    ...
    func testSQLitePersistsMemoryAndWorkflow() async throws {
        ...
        try await store.remember(key: "persisted-name", value: "Alex", sensitivity: .ordinary)
        ...
        let persistedMemories = try await reopened.search("persisted-name")
        XCTAssertEqual(persistedMemories.first?.value, "Alex")
        let persistedWorkflows = try await reopened.all()
        XCTAssertEqual(persistedWorkflows.first?.name, "work setup")
